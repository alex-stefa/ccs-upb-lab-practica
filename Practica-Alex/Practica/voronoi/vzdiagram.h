...
// Dragos Tarcatu - Modified GraphEdge structure in order to retain the sites that generated
...
public:
                 ...
                 float& xs1, float& ys1, float& xs2, float& ys2)
	{
		...
		y1 = iteratorEdges->y1;
		y2 = iteratorEdges->y2;
        xs1 = iteratorEdges->p1.x;
        ys1 = iteratorEdges->p1.y;
        xs2 = iteratorEdges->p2.x;
        ys2 = iteratorEdges->p2.y;
...
private:
	...
	void openpl();
	void line(float x1, float y1, float x2, float y2, struct Site* s1, struct Site* s2);
